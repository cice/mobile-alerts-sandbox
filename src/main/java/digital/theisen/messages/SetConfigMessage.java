...
public class SetConfigMessage extends ConfigMessage {
    ...
    public static final short COMMAND = 4;
    ...
    public byte[] getBytes() {
        MessageHeader header = new MessageHeader(COMMAND, getGatewayId(), SIZE);
        ...
        if(DataServerName != null) ByteBufferUtil.putBytes(body, DATA_SERVER_NAME_POS, DataServerName.getBytes(), DATA_SERVER_NAME_LEN);
        body.put(USE_PROXY_POS, (byte) (UseProxy ? 1 : 0));
        if(ProxyServerName != null) ByteBufferUtil.putBytes(body, PROXY_SERVER_NAME_POS, ProxyServerName.getBytes(), PROXY_SERVER_NAME_LEN);
